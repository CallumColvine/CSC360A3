...
void listRoot(FILE *diskIn, int offset, int endRootOffset){
	unsigned char status;
	int first5 = 0;
	std::cout << "offset is: " << offset << "\n" <<
		"endRootOffset is: " << endRootOffset << std::endl;
	for (offset; offset < endRootOffset; offset++)
	{
		...
		if ((int)status == 0){
			if (first5 < 5) {
				std::cout << (int)status << " " << offset << std::endl;
				first5 ++;
			} 
		} else {
			offset += 63;
...
int main(int argc, char const *argv[])
{
	...
	std::cout << "Root offset is " << rootOffset << std::endl;
...
/* 
	1. Find a spot for the file
		1.1 Go through the Space past the root
	2. Copy into this spot
